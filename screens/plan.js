...
function weekRangeLabel(s, e) {
  ...
  const sm = sd.toLocaleString('en', { month: 'short', timeZone: 'UTC' })
  const em = ed.toLocaleString('en', { month: 'short', timeZone: 'UTC' })
  return sm === em
    ? `${sm} ${sd.getUTCDate()}–${ed.getUTCDate()}`
    : `${sm} ${sd.getUTCDate()} – ${em} ${ed.getUTCDate()}`
}
...
export async function init(el) {
  screenEl = el
  // Listen for plan changes triggered by the Chat screen
  document.addEventListener('plan-updated', () => renderPlan())
}
...
async function renderPlan() {
  if (!screenEl) return
  screenEl.innerHTML = `<div class="loading-row"><div class="spinner"></div>Loading plan…</div>`
  ...
  if (error) { screenEl.innerHTML = ''; screenEl.appendChild(buildError()); return }

  const byDate = Object.fromEntries((data || []).map(r => [r.plan_date, r]))
  const allDays = Array.from({ length: 14 }, (_, i) => {
    const date = addDays(startDate, i)
    return { date, ...(byDate[date] || null) }
  })

  screenEl.innerHTML = ''
  if (!data?.length) { screenEl.appendChild(buildEmpty()); return }

  for (let w = 0; w < 2; w++) {
    screenEl.appendChild(buildWeek(allDays.slice(w * 7, w * 7 + 7)))
  }
}
...
function buildDayRow(day) {
  ...
  const { dow, date } = formatDayDate(day.date)
  const hasRecipe  = !!day.cook_source
  const recipeName = day.recipes?.name || (day.cook_source === 'freezer_stash' ? 'From Freezer' : null)
  ...
  const badge = hasRecipe ? buildSourceBadge(day.cook_source) : null
...
function buildSubline(day) {
  ...
  if (protein === 'vegetarian') parts.push('Veggie')
  else if (protein)             parts.push(cap(protein))
  if (day.recipes?.cooking_method === 'slow_cook') parts.push('Slow cook')
...
function buildSourceBadge(source) {
  ...
  el.textContent = cfg.label; el.title = cfg.title
  return el
}
...
async function onGenerate() {
  ...
  try {
    ...
    if (json.unresolved?.length)
      setTimeout(() => toast(`${json.unresolved.length} day(s) need your input — check Chat`, { duration: 5000 }), 600)
    await renderPlan()
  } catch (e) {
    ...
    generating = false
    const btn = document.getElementById('btn-generate')
    if (btn) { btn.disabled = false; btn.innerHTML = sparklesIcon() }
  }
}

function onDayTap(day) {
  toast(`Open Chat to change ${day.recipes?.name || 'this day'}`)
}
